...
function ModuloCursos({ data, search, setSearch, openNew, openEdit, deleteItem, markClosed, getResponsableName, tableLoading }: any) {
  ...
  const columns = [
    ...
    { key: "origen", label: "Origen", render: (r: Curso) => <Badge label={r.origen} colorClass="bg-slate-200 text-slate-700" /> },
    { key: "prioridad", label: "Prioridad", render: (r: Curso) => <Badge label={r.prioridad} colorClass={prioridadColor[r.prioridad] || ""} /> },
    { key: "estado", label: "Estado", render: (r: Curso) => <Badge label={r.estado} colorClass={estadoColor[r.estado] || ""} /> },
    { key: "fechaRequerida", label: "Fecha req.", render: (r: Curso) => toDDMMYYYY(r.fechaRequerida) },
    { key: "semaforo", label: "Semáforo", render: (r: Curso) => <SemaforoBadge fecha={r.fechaProximaAccion || r.fechaRequerida} /> },
    { key: "bloqueadoPor", label: "Bloqueo", render: (r: Curso) => r.bloqueadoPor !== "Sin bloqueo" ? <Badge label={r.bloqueadoPor} colorClass="bg-red-100 text-red-700" /> : "-" },
    { key: "responsable", label: "Resp.", render: (r: Curso) => getResponsableName(data, r.responsableId) },
  ];

  const activeFilters = [filtroPrioridad, filtroEstado, filtroOrigen, filtroSemaforo].filter(Boolean).length;
  ...
  return (
    <div className="space-y-5">
      <ModuleHeader
        ...
        searchPlaceholder="Buscar curso o proveedor..."
        filterPanel={
          <FilterPanel
            activeCount={activeFilters}
            onClear={() => { setFiltroPrioridad(""); setFiltroEstado(""); setFiltroOrigen(""); setFiltroSemaforo(""); }}
          >
            <Select value={filtroPrioridad} onChange={setFiltroPrioridad} options={PRIORIDADES} placeholder="Prioridad" />
            <Select value={filtroEstado} onChange={setFiltroEstado} options={ESTADOS_CURSO} placeholder="Estado" />
            <Select value={filtroOrigen} onChange={setFiltroOrigen} options={ORIGENES_CURSO} placeholder="Origen" />
            <Select value={filtroSemaforo} onChange={setFiltroSemaforo} options={["Vencido", "Vence hoy", "1-3 días", "4-7 días", "Sin urgencia", "Sin fecha"]} placeholder="Semáforo" />
          </FilterPanel>
        }
        actions={<button onClick={() => openNew("cursos")} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-700 transition-colors shadow-sm">+ Agregar nuevo</button>}
      />
      <Table
        columns={columns}
        rows={filtered}
